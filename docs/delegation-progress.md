...
| T04 合并后一致性与文档校正 | reviewed+committed | `main` | `df1d91b` | README 状态段改为“M0 能力检测 + M1/M2 库层已落地（未接入 CLI）”，并列出仍未完成的出口项，避免把库层进度误报为里程碑完成。 |
| T05 M3 关联引擎核心（外部模式 + 辅助模式 + 证据图） | running | `pi/m3-correlate` | - | 依赖 M1/M2 已合并的库 API；不接入 CLI、不做 eBPF、不做因果推断。 |
| T06 M3 Docker 任务映射（可注入查询接口） | running | `pi/m3-container` | - | 复用 M0 的“可注入 Host + fake”模式；测试不调用真实 docker。 |
...
| 任务 | 文件边界 | 说明 |
|---|---|---|
| T05 关联引擎 | `src/agent_probe/correlate/`、`tests/correlate/`、`docs/03-correlation.md` | 任务模型、证据图、外部/辅助两种关联模式、置信度与歧义、可序列化结果、消融开关。 |
| T06 容器映射 | `src/agent_probe/container/`、`tests/container/`、`docs/03b-container-mapping.md` | 标签→容器 ID→宿主 PID/cgroup→挂载视图；可注入 `ContainerQuery`，测试用 fake。 |

两者按“结构化协议”解耦：T05 自行定义 `ContainerResolver` Protocol，**不 import**
`agent_probe.container`；T06 的公开 API 在 brief 中冻结。这样两个 worktree 可并行且合并无冲突。
...
- 计划中的 M4–M6（审计报告、受限执行控制、对照实验）尚未开始；M3 正在进行（T05/T06）。
- 环境提示：`ms_pointcloud_midterm` 中**没有 PyYAML**，因此 M4 的“YAML 规则”要么先引入经论证的依赖，
